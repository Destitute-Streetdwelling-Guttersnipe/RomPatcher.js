	...
	load:function(){
		if(typeof localStorage!=='undefined' && localStorage.getItem('rompatcher-js-settings')){
			try{
				...
				if(loadedSettings.patchFileInUrl===true){
					this.patchFileInUrl=loadedSettings.patchFileInUrl;
					el('switch-patch-file-in-url').className='switch enabled';
				}
			}catch(ex){
...
addEvent(window,'load',function(){
	...
	addEvent(el('switch-patch-file-in-url'), 'click', function(){
...
const realSaveAs = saveAs
saveAs = function(blob, fileName) {
	if (!AppSettings.patchFileInUrl) {
		return realSaveAs(blob, fileName)
	}
	return base64_arraybuffer(blob).then(function(dataInBase64) {
		const PATCHER = [{ name: fileName, file: 'data:;base64,' + dataInBase64 }]
		el('patch-file-in-url').value = '#' + JSON.stringify({PATCHER: PATCHER})
		el('patch-file-in-url').style.display = 'inline'
	})
}

// ref: ArrayBufferToBase64.js https://gist.github.com/fonsp/a4a2d127f35e72deec14227c1bdf3d04
const base64_arraybuffer = async (data) => {
    ...
    // remove "data:application/octet-stream;base64," from the data URI
